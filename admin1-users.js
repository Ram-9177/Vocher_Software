(function(){
  ...
  function fillCollegeSelect(colleges){
    ...
    if (selNew) {
      const currentNew = selNew.value || 'smgg';
    ...
    if (selEdit) {
      const currentEdit = selEdit.value || 'smgg';
  ...
  window.loadUserToEdit = function(username) {
    ...
    document.getElementById('LIVE_EDIT_COLLEGE').value = u.college || 'smgg';
  ...
  window.renderCollegesLive=async function(){
    ...
    try{
      ...
      body.innerHTML=colleges.map(function(c){
        ...
        const action=code==='smgg'?'<span style="font-size:11px;color:var(--G600)">Main</span>':'<button class="btn '+(status==='active'?'br':'bp')+' bsm" onclick="toggleCollegeLive(\''+code+'\',\''+next+'\')" title="'+(status==='active'?'Disable':'Activate')+'">'+(status==='active'?'Disable':'Activate')+'</button>';
