...
const API_VERSION = 'voucher-api-old-ui-v5';
...
async function handle(context) {
  ...
  if (action === 'signup') return await signup(env.DB, body, ip);
  if (action === 'login') return await login(env.DB, request, body, ip);
  if (action === 'logout') return await logout(env.DB, request, body.token);
  if (action === 'resetPassword') return await resetPasswordLegacy(env.DB, body, ip);
...
function actualUsername(name) {
  const u = norm(name);
  if (u === 'admin1') return 'admin';
  if (u === 'admin2') return 'user2';
  if (u === 'admin3') return 'user3';
  return u;
}
function uiUsername(name) {
  const u = norm(name);
  if (u === 'admin') return 'admin1';
  if (u === 'user2') return 'admin2';
  if (u === 'user3') return 'admin3';
  return u;
}
...
function cookieToken(request, bodyToken) {
  if (bodyToken) return String(bodyToken);
  const c = request.headers.get('cookie') || '';
  const m = c.match(/(?:^|;\s*)SMV_SESSION=([^;]+)/);
  return m ? decodeURIComponent(m[1]) : '';
}
function sessionCookie(token, maxAgeSeconds) {
  return 'SMV_SESSION=' + encodeURIComponent(token || '') + '; Path=/; Max-Age=' + Number(maxAgeSeconds || 0) + '; HttpOnly; Secure; SameSite=Lax';
}
function isLegacyAdminName(username) { return ['admin1','admin2','admin3','admin','user2','user3'].indexOf(norm(username)) !== -1; }
...
async function listAdmins(DB, body) {
  ...
  (r.results || []).forEach(function (u) {
    ...
    if (mapped === 'admin1') out.push('admin1');
    else if ((u.college || 'smg') === college && ['admin2','admin3'].indexOf(mapped) !== -1) out.push(mapped);
  });
  return send({ usernames: Array.from(new Set(out)), version: API_VERSION });
}

async function signup(DB, body, ip) {
  const requested = norm(body.username);
  if (!isLegacyAdminName(requested)) throwError('Only admin1, admin2, admin3 are allowed.', 400);
  const username = actualUsername(requested);
  const existing = await DB.prepare('SELECT username FROM users WHERE username=?').bind(username).first();
  if (existing) throwError('Account already registered. Use Sign In.', 409);
  const password = String(body.password || '');
  const passwordHash = clean(body.passwordHash || '', 200);
  if (!password && !/^[a-f0-9]{64}$/i.test(passwordHash)) throwError('Password is required', 400);
  const hp = password ? await hashPassword(password) : { salt: LEGACY_SHA256, hash: passwordHash };
  ...
  await audit(DB, 'system', 'signup', 'user', username, JSON.stringify({ requested: requested, college: college, role: role }), ip);
  return send({ ok: true, version: API_VERSION });
}

async function login(env, request, body, ip) {
  const requested = norm(body.username);
  const candidates = Array.from(new Set([actualUsername(requested), requested]));
  let user = null;
  for (const c of candidates) {
    user = await env.DB.prepare('SELECT * FROM users WHERE username=?').bind(c).first();
    if (user) break;
  }
  ...
  await env.DB.prepare('INSERT INTO sessions(token,username,expires_at,created_at) VALUES(?,?,?,?)').bind(token, user.username, exp, now()).run();
  await env.DB.prepare('UPDATE users SET last_login=?,updated_at=? WHERE username=?').bind(now(), now(), user.username).run();
  await audit(env.DB, user.username, 'login', 'user', user.username, 'Login successful', ip);
  return send({ token: token, user: publicUser(user), version: API_VERSION }, 200, { 'set-cookie': sessionCookie(token, SESSION_DAYS * 86400) });
}

async function verifyLogin(body, user) {
  const password = String(body.password || '');
  const passwordHash = clean(body.passwordHash || '', 200);
  if (user.password_salt === LEGACY_SHA256) {
    if (passwordHash && safeEqual(passwordHash, user.password_hash)) return true;
    if (password) {
      const h = await sha256Hex(password);
      return safeEqual(h, user.password_hash);
    }
    return false;
  }
  if (!password) return false;
  return await verifyPassword(password, user.password_salt, user.password_hash);
}

async function logout(DB, request, bodyToken) {
  const token = cookieToken(request, bodyToken);
  if (token) await DB.prepare('DELETE FROM sessions WHERE token=?').bind(token).run();
  return send({ ok: true, version: API_VERSION }, 200, { 'set-cookie': sessionCookie('', 0) });
}
async function requireSession(DB, request, bodyToken) {
  const token = cookieToken(request, bodyToken);
...
async function resetPasswordLegacy(DB, body, ip) {
  const requested = norm(body.username);
  if (!isLegacyAdminName(requested)) throwError('Invalid username', 400);
  const targetUsername = actualUsername(requested);
  const target = await DB.prepare('SELECT * FROM users WHERE username=?').bind(targetUsername).first();
  if (!target) throwError('No account for ' + requested + ' at this institution.', 404);

  if (targetUsername !== 'admin') {
    const admin = await DB.prepare('SELECT * FROM users WHERE username=?').bind('admin').first();
    if (!admin) throwError('No admin1 account exists at this institution.', 404);
    const authOk = await verifyLogin({ password: String(body.authPassword || ''), passwordHash: clean(body.authHash || '', 200) }, admin);
    if (!authOk) throwError("Enter admin1's current password to authorize reset.", 403);
  }

  const password = String(body.password || '');
  const passwordHash = clean(body.passwordHash || '', 200);
  if (!password && !/^[a-f0-9]{64}$/i.test(passwordHash)) throwError('Invalid password', 400);
  const hp = password ? await hashPassword(password) : { salt: LEGACY_SHA256, hash: passwordHash };
  await DB.prepare('UPDATE users SET password_salt=?,password_hash=?,updated_at=? WHERE username=?').bind(hp.salt, hp.hash, now(), targetUsername).run();
  await DB.prepare('DELETE FROM sessions WHERE username=?').bind(targetUsername).run();
  await audit(DB, 'admin1', 'reset_password', 'user', targetUsername, 'Password reset and sessions revoked', ip);
  return send({ ok: true, version: API_VERSION });
}
...
async function listVouchers(DB, user, body) {
  ...
  let q;
  if (user.role === 'admin') q = DB.prepare('SELECT * FROM vouchers WHERE deleted_at IS NULL AND college=? ORDER BY date DESC,id DESC').bind(college);
  else q = DB.prepare('SELECT * FROM vouchers WHERE deleted_at IS NULL AND college=? AND created_by=? ORDER BY date DESC,id DESC').bind(college, user.username);
  const r = await q.all();
  return send({ vouchers: (r.results || []).map(voucherToOld), version: API_VERSION });
}
function voucherToOld(v) {
  const dateISO = isoFromAny(v.date);
  return {
    id: Number(v.id || 0),
    voucherNo: v.voucher_no || '',
    voucher_no: v.voucher_no || '',
    date: dmyFromIso(dateISO),
    dateISO: dateISO,
    type: v.type || 'debit',
    college: v.college || 'smg',
    head: v.head || '',
    acName: v.ac_name || '',
    ac_name: v.ac_name || '',
    ...
    remarks: v.remarks || '',
    createdBy: uiUsername(v.created_by || ''),
    created_by: v.created_by || '',
    createdAt: v.created_at || '',
    created_at: v.created_at || '',
    _u: v.updated_at || v.created_at || '',
    party: v.paid_to || v.received_from || v.ac_name || ''
  };
}
...
function dmyFromIso(s) {
  if (/^\d{4}-\d{2}-\d{2}$/.test(s)) { const p = s.split('-'); return p[2] + '-' + p[1] + '-' + p[0]; }
  return s;
}
function normalizeVoucher(v, user) {
  const type = clean(v.type, 20);
  if (['debit','onaccount','credit'].indexOf(type) === -1) throwError('Invalid voucher type', 400);
  return {
    college: allowedCollege(user, v.college),
    type: type,
    date: isoFromAny(v.dateISO || v.date),
    head: clean(v.head, 250),
    ac_name: clean(v.ac_name || v.acName, 250),
    received_from: clean(v.received_from || v.receivedFrom, 250),
    paid_to: clean(v.paid_to || v.paidTo, 250),
    towards: clean(v.towards, 500),
    amount: amount(v.amount),
    amt_words: clean(v.amt_words || v.amtWords, 500),
    mode: clean(v.mode || 'Cash', 50),
    cheque: clean(v.cheque, 120),
    prep_by: clean(v.prep_by || v.prepBy, 120),
    checked_by: clean(v.checked_by || v.checkedBy, 120),
...
async function deleteVoucher(DB,user,id,ip) {
  id = Number(id || 0); if(!id) throwError('Invalid voucher id',400);
...
async function listHeads(DB,user,body) {
  const college = allowedCollege(user, body.college);
  const r = await DB.prepare('SELECT * FROM account_heads WHERE active=1 AND college=? ORDER BY name').bind(college).all();
  return send({heads:r.results || [],version:API_VERSION});
}
async function addHead(DB,user,body,ip) {
  const name = clean(body.name,250); if(!name) throwError('Head name required',400);
  const type = ['debit','onaccount','credit','common'].indexOf(body.type) !== -1 ? body.type : 'common';
  const college = allowedCollege(user,body.college);
  ...
  await audit(DB,user.username,'add_account_head','account_head',name,JSON.stringify({type:type,college:college}),ip);
  return await listHeads(DB,user,body);
}
async function listUsers(DB) {
  const r = await DB.prepare('SELECT username,role,status,college,created_at,updated_at,last_login FROM users ORDER BY username').all();
  return send({users:(r.results || []).map(function(u){ return Object.assign({}, u, { username: uiUsername(u.username) }); }),version:API_VERSION});
}
async function createUser(DB,actor,body,ip) {
  const username=actualUsername(body.username); if(!/^[a-z0-9._-]{3,32}$/.test(username)) throwError('Username must be 3-32 letters/numbers',400);
  const password=String(body.password || ''); if(password.length<6) throwError('Password must be at least 6 characters',400);
  const existing=await DB.prepare('SELECT username FROM users WHERE username=?').bind(username).first(); if(existing) throwError('Username already exists',409);
  const role=body.role==='admin'?'admin':'user', college=clean(body.college || 'smg',20), hp=await hashPassword(password);
  await DB.prepare('INSERT INTO users(username,password_salt,password_hash,role,status,college,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)').bind(username,hp.salt,hp.hash,role,'active',college,now(),now()).run();
...
async function setUserStatus(DB,actor,body,ip) {
  ...
  if(username==='admin') throwError('Main admin cannot be blocked',400);
...
async function listAudit(DB) {
  const r = await DB.prepare('SELECT * FROM audit_logs ORDER BY id DESC LIMIT 500').all();
  return send({logs:r.results || [],version:API_VERSION});
}
...
async function hashPassword(password, saltB64) {
  ...
  const bits = await crypto.subtle.deriveBits({ name:'PBKDF2', hash:'SHA-256', salt:salt, iterations:PBKDF2_ITERATIONS }, key, 256);
