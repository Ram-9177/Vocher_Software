...
function publicUser(u) {
  ...
  return {
    ...
    fullName: u.username === 'baji' ? (u.full_name || 'Baji') : (isMain ? 'Main Administrator' : (u.full_name || '')),
...
function canManageTarget(actor, target) {
  ...
  if (!target || isSuperAdmin(target)) return false;
...
function assertManageTarget(actor, target) {
  ...
  if (target.custom_role === 'head' && !isSuperAdmin(actor) && actor.custom_role !== 'head') {
...
async function ensureSchema(DB, env) {
  ...
  await DB.prepare("CREATE TABLE IF NOT EXISTS vouchers (id INTEGER PRIMARY KEY AUTOINCREMENT,voucher_no TEXT,college TEXT NOT NULL,type TEXT NOT NULL CHECK(type IN ('debit','onaccount','credit')),date TEXT NOT NULL,head TEXT NOT NULL,ac_name TEXT,received_from TEXT,paid_to TEXT,recipient_phone TEXT,reversal_date TEXT,towards TEXT NOT NULL,amount INTEGER NOT NULL,amt_words TEXT,mode TEXT,cheque TEXT,prep_by TEXT,checked_by TEXT,remarks TEXT,created_by TEXT NOT NULL,created_at TEXT NOT NULL,updated_by TEXT,updated_at TEXT NOT NULL,deleted_at TEXT,deleted_by TEXT)").run();
  try { await DB.prepare("ALTER TABLE vouchers ADD COLUMN recipient_phone TEXT").run(); } catch(e) {}
  try { await DB.prepare("ALTER TABLE vouchers ADD COLUMN reversal_date TEXT").run(); } catch(e) {}
...
async function saveVoucher(DB,user,v,ip) {
  ...
  const ins=await DB.prepare('INSERT INTO vouchers(college,type,date,head,ac_name,received_from,paid_to,recipient_phone,reversal_date,towards,block,amount,amt_words,mode,cheque,prep_by,checked_by,remarks,created_by,created_at,updated_by,updated_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)').bind(row.college,row.type,row.date,row.head,row.ac_name,row.received_from,row.paid_to,row.recipient_phone,row.reversal_date,row.towards,row.block,row.amount,row.amt_words,row.mode,row.cheque,row.prep_by,row.checked_by,row.remarks,user.username,now(),user.username,now()).run();
...
async function listBlocks(DB,user,body){const r=await DB.prepare('SELECT * FROM blocks WHERE active=1 ORDER BY name').all();return send({blocks:r.results||[],version:API_VERSION});}
...
async function updateUserPermissions(DB,actor,body,ip){
  ...
  if(isSuperAdmin(username))throwError('Super administrators cannot be permission-reduced or modified.',400);
  ...
  const customRole = body.role === 'head' ? 'head' : '';
  const role = (body.role === 'admin' || body.role === 'head') ? 'admin' : 'user';
  if (role === 'admin' && target.role !== 'admin' && !isSuperAdmin(actor) && !hasPermission(actor, 'create_admin')) {
    throwError('Access denied. Missing create_admin permission to promote to admin.', 403);
  }
  const fullName=clean(body.fullName||body.full_name||'',200);
  const college=clean(body.college||'smgg',20);
...
async function resetUserPassword(DB,actor,body,ip){
  ...
  const mustChange = isSuperAdmin(username) ? 0 : 1;
...
async function deleteUser(DB,actor,body,ip){
  const username=actualUsername(body.username);
  if(isSuperAdmin(username)||username==='admin1')throwError('Main admins cannot be deleted',400);
