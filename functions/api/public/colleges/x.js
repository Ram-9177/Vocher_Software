...
async function handle(context){
  ...
  const session=await requireAdmin(env.DB,request,body.token);
  if(action==='listColleges')return await listColleges(env.DB);
  if(action==='createCollege')return await createCollege(env.DB,session.user,body,ip);
  if(action==='setCollegeStatus')return await setCollegeStatus(env.DB,session.user,body,ip);
...
async function requireAdmin(DB,request,bodyToken){
  ...
  if(row.role!=='admin')throwError('Access denied. Only admin1 can manage colleges.',403);
  return{user:row};
}
